...
t_1d_char_array * c_split_str_remainder(const char * a_input, char a_delim, int a_column_count, int a_string_size)
{
    int f_i = 0;
    int f_current_string_index = 0;
    int f_current_column = 0;
    ...
    while(1)
    {
        if((f_current_column < (a_column_count - 1)) && (a_input[f_i] == a_delim))
        {
            f_result->array[f_current_column][f_current_string_index] = '\0';
            f_current_column++;
            f_current_string_index = 0;                        
...
{
    t_key_value_pair * f_result = (t_key_value_pair*)malloc(sizeof(t_key_value_pair));
    int f_i = 0;
    f_result->key_len = 0;
    f_result->val_len = 0;
    int f_stage = 0;
...
{
    t_pydaw_line_split * f_result = (t_pydaw_line_split*)malloc(sizeof(t_pydaw_line_split));
    f_result->count = 1;
