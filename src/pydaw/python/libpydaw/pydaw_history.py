...
class pydaw_history:
    ...
    def commit(self, a_commit):
        ...
        for f_file in a_commit.files:
            f_cursor.execute(
                "INSERT INTO pydaw_diffs VALUES(?, ?, ?, ?, ?, ?)",
    ...
    def revert_to(self, a_timestamp):
        ...
        for f_row in f_rows:
            ...
            f_diff_arr.append(pydaw_history_file(
                f_row[2], f_row[1], f_row[3], f_row[4], f_row[5]))
        f_commit = pydaw_history_commit(
            f_diff_arr, "Revert to commit {}".format(a_timestamp))
...
class pydaw_history_log_widget(QtGui.QWidget):
    def __init__(self, a_history_db, a_ui_callback=None):
        ...
        self.revert_to_action = QtGui.QAction(
            "revert project to this point", self)
    ...
    def populate_table(self):
        ...
        for f_commit in f_arr:
            ...
            self.table_widget.setItem(
                f_index, 0, QtGui.QTableWidgetItem(str(f_commit[0])))
            self.table_widget.setItem(
                f_index, 1, QtGui.QTableWidgetItem(str(f_commit[1])))
        self.table_widget.resizeColumnsToContents()

    def on_show(self):
        #TODO:  Create a unified diff of all files...
        f_timestamp = int(self.table_widget.item(
            self.table_widget.currentRow(), 0).text())
        f_result = ""
