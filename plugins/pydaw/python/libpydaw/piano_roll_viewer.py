...
class note_item(QtGui.QGraphicsRectItem):
    def __init__(self, a_length, a_note_height, a_value_snap, a_piano_width, a_header_height, a_padding):
        QtGui.QGraphicsRectItem.__init__(self, 0, 0, a_length, a_note_height)
        self.setFlag(QtGui.QGraphicsItem.ItemIsMovable)
        self.setFlag(QtGui.QGraphicsItem.ItemSendsGeometryChanges)
        self.grid_snap = True
        self.value_snap = a_value_snap
        self.note_height = a_note_height
        self.piano_width = a_piano_width + a_padding
        self.header_height = a_header_height
    ...
    def mouseMoveEvent(self, a_event):
        ...
        f_pos_x = self.pos().x()
        f_pos_y = self.pos().y()
        if f_pos_x < self.piano_width:
            f_pos_x = self.piano_width
        if f_pos_y < self.header_height:
            f_pos_y = self.header_height
        f_pos_y = int((f_pos_y - self.o_pos.y())/self.note_height) * self.note_height
        if self.grid_snap:
            f_pos_x = int((f_pos_x - self.o_pos.x())/self.value_snap) * self.value_snap
        else:
...
class piano_roll_viewer(QtGui.QGraphicsView):
    ...
    def draw_piano(self):
        ...
        #f_key = piano_key_item(self.piano_width, self.note_height, self.piano)
        #f_key.setBrush(QtGui.QColor(255,255,255))
        #f_octave = QtGui.QGraphicsSimpleTextItem("C8", f_key)
        #f_octave.setPos(3, 0)
        #f_octave.setFont(f_piano_label)
        #for i in range(1, self.total_notes+1):
        #    f_key = piano_key_item(self.piano_width, self.note_height, self.piano)
        #    f_key.setPos(0, self.note_height*(i-1))
        #    if i in f_black_notes:
        #        f_key.setBrush(QtGui.QColor(0,0,0))
        #    else:
        #        f_key.setBrush(QtGui.QColor(255,255,255))
        #    if (i % 12) == 0:
        #        f_octave = QtGui.QGraphicsSimpleTextItem("C%d" % (f_octave_number), f_key)
        #        f_octave.setPos(3, 0)
        #        f_octave.setFont(f_piano_label)
        #        f_octave_number -= 1
        #        if i:
        #            for n in range(len(f_black_notes)):
        #                f_black_notes[n] += 12
    ...
    def draw_note(self, a_note, a_start, a_length, a_velocity, a_color):
        ...
        f_note_item = note_item(f_length, self.note_height, self.value_width, self.piano_width, self.header_height, self.padding)
        f_note_item.setPos(f_start, f_note)
        f_note_item.setBrush(a_color)
...
if __name__ == '__main__':
    ...
    view.draw_note(60, 2.0625, 0.25, 127, pydaw_track_gradients[4]) 
    view.draw_note(120, 1, 0.5, 100, pydaw_track_gradients[4])
    view.show()
    sys.exit(app.exec_())
