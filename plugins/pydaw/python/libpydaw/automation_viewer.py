...
class automation_viewer(QtGui.QGraphicsView):
    ...
    def sceneMouseDoubleClickEvent(self, a_event):
        ...
        f_time = (f_pos_x - self.axes_size)/self.beat_width
        f_value = self.steps - ((f_pos_y - self.axes_size) * self.steps / self.viewer_height)
    ...
    def draw_axes(self):
        self.x_axis = QtGui.QGraphicsRectItem(0, 0, self.viewer_width, self.axes_size)
        self.x_axis.setPos(self.axes_size, 0)
        self.scene.addItem(self.x_axis)
        self.y_axis = QtGui.QGraphicsRectItem(0, 0, self.axes_size, self.viewer_height)
        self.y_axis.setPos(0, self.axes_size)
    ...
    def draw_grid(self):
        ...
        for i in range(1,6):
            ...
            f_line.setPos(self.axes_size,self.viewer_height*(i-1)/4)
            if i % 2:
        ...
        for i in range(0, int(self.item_length)+1):
            f_beat = QtGui.QGraphicsLineItem(0, 0, 0, self.viewer_height+self.axes_size-f_beat_pen.width(), self.x_axis)
            ...
            if i < self.item_length:
                ...
                for j in range(0, self.grid_div):
                    ...
                    if float(j) == self.grid_div / 2.0:
                        ...
                        f_line.setPos((self.beat_width*i)+(self.value_width*j), self.axes_size)
                    else:
                        f_line.setPos((self.beat_width*i)+(self.value_width*j), self.axes_size)
    ...
    def draw_point(self, a_time, a_value):
        ...
        f_time = self.axes_size + self.beat_width * a_time
        f_value = self.axes_size +  self.viewer_height/self.steps * (self.steps-a_value)
