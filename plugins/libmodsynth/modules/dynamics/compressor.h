...
#ifndef PYDAW_COMPRESSOR_H
#define	PYDAW_COMPRESSOR_H
...
#define CMP_HOLD_TIME_DIVISOR 500.0f
...
typedef struct
{
    ...
    float last_thresh, last_ceiling, last_release, last_attack;
    float autogain;
...
t_cmp_compressor * g_cmp_get(float);
void v_cmp_set(t_cmp_compressor*,float, float, float, float);
void v_cmp_run(t_cmp_compressor*,float, float);

void v_cmp_set(t_cmp_compressor*a_lim, float a_thresh, float a_ceiling, float a_attack, float a_release)
{
    ...
    if(a_release != (a_lim->last_release))
    {
        ...
        a_lim->r = ((a_lim->sr_recip) / (a_lim->release)) * CMP_HOLD_TIME_DIVISOR;        
        a_lim->last_release = a_release;
    }
}

void v_cmp_run(t_cmp_compressor *a_lim, float a_in0, float a_in1)
{
...
t_cmp_compressor * g_cmp_get(float srate)
{
    t_cmp_compressor * f_result;
    if(posix_memalign((void**)&f_result, 16, sizeof(t_cmp_compressor)) != 0)
    {
        return 0;
    }
    
    f_result->holdtime = ((int)(srate/CMP_HOLD_TIME_DIVISOR));
    ...
    f_result->last_attack = 1234.4532f;
